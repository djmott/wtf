...
namespace wtf{
  ...
  namespace _{
    namespace policy_list_impl{
      ...
      template <template <typename, typename> typename, template <typename, typename> typename> struct is_same_policy : std::false_type{};
      template <template <typename, typename> typename _PolicyT> struct is_same_policy<_PolicyT, _PolicyT> : std::true_type{};


      //append a policy_list to an existing policy_list
      template <typename _AppendList, typename _SourceList> struct append_list;
      template <template <typename, typename> typename ... _AppendListTs, template <typename, typename> typename ... _SourceListTs>
      ...
      template <template <typename, typename> typename _TargetT, template <typename, typename> typename ... _Policies> struct exists;
      template <template <typename, typename> typename _TargetT> struct exists<_TargetT> : std::false_type{};

      template <template <typename, typename> typename _TargetT, template <typename, typename> typename ... _TailT>
      struct exists<_TargetT, _TargetT, _TailT...> : std::true_type{};

      template <template <typename, typename> typename _TargetT, template <typename, typename> typename _HeadT, template <typename, typename> typename ... _TailT>
      struct exists<_TargetT, _HeadT, _TailT...> : exists<_TargetT, _TailT...>{};

      //remove the first occurrence of a policy in a parameter pack
      template <template <typename, typename> typename _TargetT, template <typename, typename> typename ... _Policies> struct remove;
      template <template <typename, typename> typename _TargetT> struct remove<_TargetT>{ using type = policy_list<>; };
      template <template <typename, typename> typename _TargetT, template <typename, typename> typename _HeadT, template <typename, typename> typename ... _TailT>
      ...
      template <template <typename, typename> typename _TargetT, template <typename, typename> typename ... _Policies> struct remove_all;
      template <template <typename, typename> typename _TargetT> struct remove_all<_TargetT>{ using type = policy_list<>; };



      template <bool, template <typename, typename> typename _TargetT, template <typename, typename> typename ... _Policies> struct remove_all_b;
      ...
      template <template <typename, typename> typename _TargetT, typename _PolicyListT> struct remove_all_list;
      template <template <typename, typename> typename _TargetT, template <typename, typename> typename ... _Policies>
      ...
      template <template <typename, typename> typename ... _Policies> struct reverse;
      template <> struct reverse<>{ using type = policy_list<>; };

      template <template <typename, typename> typename _HeadT, template <typename, typename> typename ... _TailT>
      ...
      template <template <typename, typename> typename ... _Policies>
      ...
      //remove dups
      template <template <typename, typename> typename ... _Policies> struct dedup;
      template <template <typename, typename> typename _HeadT, template <typename, typename> typename ... _TailT>
  ...
  template <> struct policy_list<>{
    template<template <typename, typename> typename ... _AppendT>
    using append = policy_list<_AppendT...>;

    template <template <typename, typename> typename _TargetT>
  ...
  struct policy_collector<policy_list<_HeadT, _TailT...>>{
    ...
    using full_reqs = typename policy_collector<tail_with_reqs>::type;
  ...
  template <typename _ImplT, template <typename, typename> typename _HeadT, template <typename, typename> typename ... _TailT>
  struct construct_hierarchy<_ImplT, policy_list<_HeadT, _TailT...>>{
    ...
    using type = _HeadT<tail_hierarchy, _ImplT>;
  };

  template <typename _ImplT, template <typename, typename> typename ... _Policies> struct normalized_policies{
