...
namespace wtf{
  ...
  struct brush : std::unique_ptr<HBRUSH__, void(*)(HBRUSH)>{
    ...
    static brush system_brush(system_colors color){
      return brush(wtf::exception::throw_lasterr_if(GetSysColorBrush(static_cast<int>(color)), [](HBRUSH b){ return !b; }), [](HBRUSH h){});
    }
    static brush solid_brush(COLORREF color){
      return brush(wtf::exception::throw_lasterr_if(CreateSolidBrush(color), [](HBRUSH b){return !b; }), [](HBRUSH b){::DeleteObject(b); });
    }

    static brush hatch_brush(hatch_style style, COLORREF color){
      return brush(wtf::exception::throw_lasterr_if(CreateHatchBrush(static_cast<int>(style), color), [](HBRUSH b){return !b; }), [](HBRUSH b){::DeleteObject(b); });
    }
